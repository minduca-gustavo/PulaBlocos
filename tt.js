...
async function iniciar() {
    ...
    criaSubTitulo({ id: 'subtitulo-backup', texto: 'Backup', ancestral: 'app' })
    criaDiv({ id: 'container-backup', ancestral: 'app', gap: '6px' })
    criaBotao({
        id: 'botao-exportar',
        texto: '⬇ Exportar backup (.json)',
        cor: 'secundaria',
        ancestral: 'container-backup',
        acao: exportarBackup,
    })
    criaInputArquivo({
        id: 'input-importar',
...
function mostrarAvisoBackup(texto, tipo) {
    const antigo = document.getElementById('aviso-backup')
    if (antigo) antigo.remove()
    criaCaixaDeAviso({ id: 'aviso-backup', texto, tipo, ancestral: 'container-backup' })
}

async function exportarBackup() {
    try {
        const dados = await obterArmazenamentoCompleto()
        const json = JSON.stringify(dados, null, 2)
        const blob = new Blob([json], { type: 'application/json' })
        const url = URL.createObjectURL(blob)

        const dataAtual = new Date().toISOString().slice(0, 10) // AAAA-MM-DD
        ...
        mostrarAvisoBackup('Não deu pra gerar o backup: ' + erro.message, 'erro')
    }
}

// Restaura SUBSTITUINDO todo o storage atual pelo do arquivo (é o
// comportamento esperado de "carregar backup"). Se preferir mesclar
// em vez de substituir, troque para { substituir: false } abaixo.
async function importarBackup(arquivo) {
    try {
        ...
        await restaurarArmazenamento(dados, { substituir: true })
        mostrarAvisoBackup('Backup restaurado! Feche e abra o popup de novo pra ver tudo atualizado.', 'sucesso')
    } catch (erro) {
        console.error('[Pula Blocos] falha ao importar backup:', erro)
        mostrarAvisoBackup('Arquivo inválido: ' + erro.message, 'erro')
    }
}